...
<section class="relative h-screen w-full overflow-hidden" x-data="{ safarisQuoteModal: false }">
    <!-- Background Image with Gradient Overlay -->
    <div class="absolute inset-0 z-0">
        <img 
            src="{{ asset('images/image-used/14.Old-Mondoro---walking-safari-elephant-encounter-timbuktu-travel.jpg') }}" 
            alt="Kilimanjaro Sunset" 
            class="w-full h-full object-cover"
        >
        <div class="absolute inset-0" style="background: linear-gradient(180deg, rgba(60, 50, 14, 1) 0%, rgba(0, 0, 0, 0) 70%);"></div>
    </div>
    ...
    <div class="relative z-10 flex flex-col items-center justify-start h-full px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div class="mt-4 flex flex-row lg:gap-8 gap-3 justify-center pt-[10%]">
            ...
            <button @click="safarisQuoteModal = true"
            ...
            <a href="{{ route('all-packages',['category'=>'safari']) }}" class="inline-block bg-white hover:bg-gray-100 text-gray-800 px-3 py-2 rounded-sm transition-colors text-center text-[15px]" style="font-family: 'brandon-bold-webfont', sans-serif; letter-spacing: 1.84px;">
                Learn More
            </a>
        </div>
        @include('partials.sections.quick-quote',['modalName'=>'safarisQuoteModal'])
    </div>
</section>